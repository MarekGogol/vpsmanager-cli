    ...
    public function rebootSSH()
    {
        if ( $this->test() )
        {
            if ( $this->restart(false) ){
                $this->response()->success('<comment>SSH has been successfully restarted.</comment>')->writeln();
            } else {
                $this->response()->message('<error>SSH could not be restarted. Please try again manually.</error>')->writeln();
            }
        } else {
            $this->response()->message('<error>SSH configuration is not correct. Please fix ssh configuration and restart ssh manually.</error>')->writeln();
        }
    }
}
